...
class Util(object):
    ...
    class lazySizeChangeTimer(Timer):
        ...
        def run(self):
            self.componentSizeUpdated(self.latelyChangedWidgets)
            self.latelyChangedWidgets.clear()
    ...
    def parseRelativeSize(cls, *args):
        ...
        if _1 == 1:
            if isinstance(_0[0], UIDL):
                ...
                relativeSize = cls.FloatSize(relativeWidth, relativeHeight)
                return relativeSize
            else:
                size, = _0
    ...
    def escapeHTML(cls, html):
        ...
        """
        cls.DOM.setInnerText(cls._escapeHtmlHelper, html)
        return cls.DOM.getInnerHTML(cls._escapeHtmlHelper)
    ...
    def doIE6PngFix(cls, el):
        blankImageUrl = cls.GWT.getModuleBaseURL() + 'ie6pngfix/blank.gif'
    ...
    def measureHorizontalPaddingAndBorder(cls, element, paddingGuess):
        originalWidth = cls.DOM.getStyleAttribute(element, 'width')
        originalOverflow = ''
        if BrowserInfo.get().isIE6():
            originalOverflow = cls.DOM.getStyleAttribute(element, 'overflow')
            cls.DOM.setStyleAttribute(element, 'overflow', 'hidden')
        originalOffsetWidth = element.getOffsetWidth()
        widthGuess = originalOffsetWidth - paddingGuess
        if widthGuess < 1:
            widthGuess = 1
        cls.DOM.setStyleAttribute(element, 'width', widthGuess + 'px')
        padding = element.getOffsetWidth() - widthGuess
        cls.DOM.setStyleAttribute(element, 'width', originalWidth)
        if BrowserInfo.get().isIE6():
            cls.DOM.setStyleAttribute(element, 'overflow', originalOverflow)
        return padding

    @classmethod
    def measureVerticalPaddingAndBorder(cls, element, paddingGuess):
        originalHeight = cls.DOM.getStyleAttribute(element, 'height')
        originalOffsetHeight = element.getOffsetHeight()
        widthGuess = originalOffsetHeight - paddingGuess
        if widthGuess < 1:
            widthGuess = 1
        cls.DOM.setStyleAttribute(element, 'height', widthGuess + 'px')
        padding = element.getOffsetHeight() - widthGuess
        cls.DOM.setStyleAttribute(element, 'height', originalHeight)
        return padding
    ...
    def setHeightExcludingPaddingAndBorder(cls, *args):
        ...
        if _1 == 3:
            ...
            if heightGuess < 0:
                heightGuess = 0
            cls.DOM.setStyleAttribute(element, 'height', heightGuess + 'px')
            captionOffsetHeight = cls.DOM.getElementPropertyInt(element, 'offsetHeight')
            ...
            if actualPadding != verticalPaddingBorderGuess:
                ...
                if h < 0:
                    ...
                    h = 0
                cls.DOM.setStyleAttribute(element, 'height', h + 'px')
    ...
    def setWidth(cls, widget, width):
        cls.DOM.setStyleAttribute(widget.getElement(), 'width', width)

    @classmethod
    def setHeight(cls, widget, height):
        cls.DOM.setStyleAttribute(widget.getElement(), 'height', height)
    ...
    def setWidthExcludingPaddingAndBorder(cls, *args):
        ...
        if _1 == 3:
            ...
            if widthGuess < 0:
                widthGuess = 0
            cls.DOM.setStyleAttribute(element, 'width', widthGuess + 'px')
            captionOffsetWidth = cls.DOM.getElementPropertyInt(element, 'offsetWidth')
            ...
            if actualPadding != horizontalPaddingBorderGuess:
                ...
                if w < 0:
                    ...
                    w = 0
                cls.DOM.setStyleAttribute(element, 'width', w + 'px')
            return actualPadding
        else:
            raise ARGERROR(3, 4)
    ...
    def setFloat(cls, element, value):
        if BrowserInfo.get().isIE():
            cls.DOM.setStyleAttribute(element, 'styleFloat', value)
        else:
            cls.DOM.setStyleAttribute(element, 'cssFloat', value)
    ...
    def getNativeScrollbarSize(cls):
        if cls._detectedScrollbarSize < 0:
            ...
            cls.RootPanel.getBodyElement().appendChild(scroller)
            cls._detectedScrollbarSize = scroller.getOffsetWidth() - scroller.getPropertyInt('clientWidth')
            cls.RootPanel.getBodyElement().removeChild(scroller)
        ...
        if (
            ...
            class _1_(Command):
                ...
                def execute(self):
                    ...
                    if (
                        BrowserInfo.get().isChrome() and (self.scrollleft > 0) or (self.elem.getScrollLeft() > 0)
                    ):
            ...
            _1_ = cls._1_()
            cls.Scheduler.get().scheduleDeferred(_1_)
    ...
    def runIE7ZeroSizedBodyFix(cls):
        ...
        if BrowserInfo.get().isIE7():
            offsetWidth = cls.RootPanel.getBodyElement().getOffsetWidth()
            if offsetWidth == 0:
    ...
    def shakeBodyElement(cls):
        ...
        """
        shaker = cls.Document.get().createDivElement()
        cls.RootPanel.getBodyElement().insertBefore(shaker, cls.RootPanel.getBodyElement().getFirstChildElement())
        shaker.getStyle().setPropertyPx('height', 0)
        shaker.setInnerHTML('&nbsp;')
        cls.RootPanel.getBodyElement().removeChild(shaker)
    ...
    def findWidget(cls, element, class1):
        ...
        if element is not None:
            ...
            while eventListener is None and element is not None:
                eventListener = cls.Event.getEventListener(element)
    ...
    def sinkOnloadForImages(cls, element):
        ...
        while True:
            ...
            if not (i < imgElements.getLength()):
                break
            cls.DOM.sinkEvents(imgElements.getItem(i), cls.Event.ONLOAD)
    ...
    def logVariableBurst(cls, c, loggedBurst):
        try:
            ...
            while True:
                ...
                split = loggedBurst[i].split(ApplicationConnection.VAR_FIELD_SEPARATOR)
                id = split[0]
                if curId is None:
                    curId = id
                elif not (curId == id):
    ...
    def getTouchOrMouseClientX(cls, *args):
        ...
        if _1 == 1:
            if isinstance(_0[0], Event):
                ...
                event, = _0
                return cls.getTouchOrMouseClientX(cls.Event.as_(event))
        else:
            raise ARGERROR(1, 1)
    ...
    def getTouchOrMouseClientY(cls, *args):
        ...
        if _1 == 1:
            if isinstance(_0[0], Event):
                ...
                currentGwtEvent, = _0
                return cls.getTouchOrMouseClientY(cls.Event.as_(currentGwtEvent))
        else:
            raise ARGERROR(1, 1)
    ...
    def isTouchEvent(cls, *args):
        ...
        if _1 == 1:
            if isinstance(_0[0], Event):
                ...
                event, = _0
                return cls.isTouchEvent(cls.Event.as_(event))
        else:
            raise ARGERROR(1, 1)
    ...
    def simulateClickFromTouchEvent(cls, touchevent, widget):
        ...
        createMouseUpEvent = cls.Document.get().createMouseUpEvent(0, touch.getScreenX(), touch.getScreenY(), touch.getClientX(), touch.getClientY(), False, False, False, False, cls.NativeEvent.BUTTON_LEFT)
        createMouseDownEvent = cls.Document.get().createMouseDownEvent(0, touch.getScreenX(), touch.getScreenY(), touch.getClientX(), touch.getClientY(), False, False, False, False, cls.NativeEvent.BUTTON_LEFT)
        createMouseClickEvent = cls.Document.get().createClickEvent(0, touch.getScreenX(), touch.getScreenY(), touch.getClientX(), touch.getClientY(), False, False, False, False)
        ...
        _2_ = cls._2_()
        cls.Scheduler.get().scheduleDeferred(_2_)
