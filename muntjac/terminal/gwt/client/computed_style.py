...
class ComputedStyle(object):
    ...
    def __init__(self, elem):
        """Gets this element's computed style object which can be used to
        gather information about the current state of the rendered node.

        Note that this method is expensive. Wherever possible, reuse the
        returned object.
    ...
    def getProperty(self, name):
        ...
        @return: the value of the property, normalized for across browsers
                 (each browser returns pixel values whenever possible).
        """
        JS("""
    ...
    def getMargin(self):
        """Get current margin values from the DOM. The array order is the
    ...
    def getPadding(self):
        """Get current padding values from the DOM. The array order is the
        default CSS order: top, right, bottom, left.
        """
        padding = [0, 0, 0, 0]
    ...
    def getBorder(self):
        """Get current border values from the DOM. The array order is the
        default CSS order: top, right, bottom, left.
        """
        border = [0, 0, 0, 0]
    ...
    def parseInt(cls, value):
        ...
        @return: int the value from the string before any non-numeric
                 characters. If the value cannot be parsed to a number,
                 returns C{None}.
        """
        JS("""
