...
class CSSRule(object):
    """Utility class for fetching CSS properties from DOM StyleSheets JS
    object."""
    ...
    def fetchRule(self, selector, deep):
        JS("""
            ...
            @{{self}}.@com.vaadin.terminal.gwt.client.CSSRule::rules = @com.vaadin.terminal.gwt.client.CSSRule::searchForRule(Lcom/google/gwt/core/client/JavaScriptObject;Ljava/lang/String;Z)(sheet, @{{selector}}, @{{deep}});
            return;
            }
            }
            @{{self}}.@com.vaadin.terminal.gwt.client.CSSRule::rules = [];
        """)
        pass
    ...
    def searchForRule(cls, sheet, selector, deep):
        JS("""
            ...
            break; // No need to loop other selectors for @{{self}} rule
            }
            }
    ...
    def getPropertyValue(self, propertyName):
        ...
        """
        JS("""
            var j = @{{self}}.@com.vaadin.terminal.gwt.client.CSSRule::rules.length;
            for(var i=0; i<j; i++) {
            var value = @{{self}}.@com.vaadin.terminal.gwt.client.CSSRule::rules[i].style[@{{propertyName}}];
            if(value)
            return value;
            }
            return null;
