    ...
    class ChildComponentContainerIterator(Iterator):
        ...
        def get(self, i):
            if i == 0:
                ...
                if self.widget is not None and self.caption is not None:
                    return self.caption
                else:
                    raise NoSuchElementException()
            else:
        ...
        def remove(self):
            ...
            if toRemove == 0:
                if self.widget is not None:
                    _ChildComponentContainer_this.remove(self.widget)
                elif self.caption is not None:
                    _ChildComponentContainer_this.remove(self.caption)
                else:
