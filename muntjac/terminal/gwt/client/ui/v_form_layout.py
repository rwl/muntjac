...
from gwt.ui.HTML import HTML


class VFormLayout(SimplePanel, IContainer):
    ...
    def getStylesFromUIDL(self, uidl):
        ...
        if uidl.hasAttribute('disabled'):
            styles.append(ApplicationConnection.DISABLED_CLASSNAME)
        return list(styles)
...
class VFormLayoutTable(FlexTable, ClickHandler):
    ...
    def __init__(self, fl):
        ...
        self._componentToCaption = dict()
        self._componentToError = dict()

        DOM.setElemAttribute(self.getElement(), 'cellPadding', '0')
        DOM.setElemAttribute(self.getElement(), 'cellSpacing', '0')


    def updateFromUIDL(self, uidl, client):
        margins = VMarginInfo(uidl.getIntAttribute('margins'))
        margin = self.getElement()
        self.setStyleName(margin, self._fl._CLASSNAME + '-'
        ...
        self.setStyleName(margin, self._fl._CLASSNAME + '-' + 'spacing',
        ...
        while it.hasNext():
            ...
            if caption is None:
                caption = Caption(p, client,
                        self._fl.getStylesFromUIDL(childUidl))
            ...
            self.getCellFormatter().setStyleName(i, self._COLUMN_WIDGET,
                    self._fl._CLASSNAME + '-contentcell')
            self.getCellFormatter().setStyleName(i, self._COLUMN_CAPTION,
                    self._fl._CLASSNAME + '-captioncell')

            self.setWidget(i, self._COLUMN_CAPTION, caption)
            self.setContentWidth(i)
            self.getCellFormatter().setStyleName(i, self._COLUMN_ERRORFLAG,
                    self._fl._CLASSNAME + '-errorcell')
            self.setWidget(i, self._COLUMN_ERRORFLAG, error)
            p.updateFromUIDL(childUidl, client)
            rowstyles = self._fl._CLASSNAME + '-row'

            if i == 0:
                rowstyles += ' ' + self._fl._CLASSNAME + '-firstrow'

            if not it.hasNext():
                rowstyles += ' ' + self._fl._CLASSNAME + '-lastrow'
    ...
    def setContentWidth(self, row):
        width = ''
        if not self._fl.isDynamicWidth():
            width = '100%'
    ...
    def replaceChildComponent(self, oldComponent, newComponent):
        for i in range(self.getRowCount()):
            ...
            if oldComponent == candidate:
                ...
                newCap = Caption(newComponent, self._fl._client, None)
                ...
                if error is None:
                    error = ErrorFlag()
                    self._componentToError[newComponent] = error
    ...
    def onClick(self, event):
        ...
        if caption.getOwner() is not None:
            if isinstance(caption.getOwner(), IFocusable):
                caption.getOwner().focus()
            elif (isinstance(caption.getOwner(), gwt.user.client.ui.Focusable)):
    ...
    def updateCaption(self, uidl):
        ...
        if uidl.hasAttribute('caption'):
            if self._captionText is None:
                ...
                DOM.insertChild(self.getElement(),
                        self._captionText, 0 if self._icon is None else 1)
        ...
        if uidl.getBooleanAttribute('required'):
            if self._requiredFieldIndicator is None:
                ...
                DOM.setElemAttribute(self._requiredFieldIndicator,
                        'className', 'v-required-field-indicator')
                DOM.appendChild(self.getElement(),
                        self._requiredFieldIndicator)
...
class ErrorFlag(HTML):
    ...
    def updateFromUIDL(self, uidl, component):
        ...
        if (uidl.hasAttribute('error')
                and not uidl.getBooleanAttribute('hideErrors')):
    ...
    def onBrowserEvent(self, event):
        ...
        if self._owner is not None:
            self._fl._client.handleTooltipEvent(event, self._owner)
