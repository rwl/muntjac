...
class VTextField(TextBoxBase, Paintable, Field, ChangeHandler, FocusHandler, BlurHandler, BeforeShortcutActionListener, KeyDownHandler):
    ...
    def __init__(self, *args):
        ...
        _1 = len(args)
        if _1 == 0:
            self.__init__(self.DOM.createInputText())
        elif _1 == 1:
            node, = _0
            ...
            if (
                ...
                self.DOM.setStyleAttribute(node, 'marginTop', '-1px')
                self.DOM.setStyleAttribute(node, 'marginBottom', '-1px')
    ...
    _TEXTCHANGE_EVENTS = (self.Event.ONPASTE | self.Event.KEYEVENTS) | self.Event.ONMOUSEUP
    ...
    class textChangeEventTrigger(Timer):
        ...
        def run(self):
            if self.isAttached():
                ...
                textChanged = self.communicateTextValueToServer()
                if textChanged:
    ...
    def updateFromUIDL(self, uidl, client):
        ...
        text = uidl.getStringVariable('text') if uidl.hasVariable('text') else None
        self.setPrompting(self._inputPrompt is not None and self._focusedTextField != self and (text is None) or (text == ''))
        if BrowserInfo.get().isFF3():
            ...
            class _1_(Command):
                ...
                def execute(self):
                    if self.prompting:
                        fieldValue = '' if self.isReadOnly() else self.inputPrompt
            ...
            self.setText(fieldValue)
        self._lastTextChangeString = self._valueBeforeEdit = uidl.getStringVariable('text')
    ...
    def onDetach(self):
        ...
        if self._focusedTextField == self:
    ...
    def detectExtraSizes(self):
        ...
        self.DOM.setElementAttribute(clone, 'id', '')
        self.DOM.setStyleAttribute(clone, 'visibility', 'hidden')
        self.DOM.setStyleAttribute(clone, 'position', 'absolute')
        # due FF3 bug set size to 10px and later subtract it from extra pixels
        self.DOM.setStyleAttribute(clone, 'width', '10px')
        self.DOM.setStyleAttribute(clone, 'height', '10px')
        self.DOM.appendChild(self.DOM.getParent(self.getElement()), clone)
        self._extraHorizontalPixels = self.DOM.getElementPropertyInt(clone, 'offsetWidth') - 10
        self._extraVerticalPixels = self.DOM.getElementPropertyInt(clone, 'offsetHeight') - 10
        self.DOM.removeChild(self.DOM.getParent(self.getElement()), clone)
    ...
    def setWordwrap(self, enabled):
        ...
        if enabled:
            ...
            self.getElement().getStyle().setOverflow(self.Overflow.AUTO)
    ...
    def onKeyDown(self, event):
        if event.getNativeKeyCode() == self.KeyCodes.KEY_ENTER:
            self.valueChange(False)
