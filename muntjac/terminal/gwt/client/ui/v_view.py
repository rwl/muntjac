...
from muntjac.terminal.gwt.client.ui.shortcut_action_handler import ShortcutActionHandler, ShortcutActionHandlerOwner
...
from muntjac.terminal.gwt.client.ui.v_notification import VNotification
...
class VView(SimplePanel, IContainer, ShortcutActionHandlerOwner, IFocusable):
#    ResizeHandler, Window.ClosingHandler):
    ...
    def __init__(self):
        ...
        # Reference to the parent frame/iframe. Null if there is no parent
        # (i)frame or if the application and parent frame are in different
        # domains.
        self._parentFrame = None

        self.myRenderSpace = MyRenderSpace()
        ...
        class _1_(ScheduledCommand):
            ...
            def execute(self):
                VView_this.windowSizeMaybeChanged(Window.getClientWidth(),
                        Window.getClientHeight())

        _1_ = _1_()
        self._delayedResizeExecutor = VLazyExecutor(200, _1_)
    ...
    def windowSizeMaybeChanged(self, newWidth, newHeight):
        ...
        if changed:
            ...
            self._connection.runDescendentsLayout(self)
    ...
    def isEmbedded(self):
        """Returns true if the body is NOT generated, i.e if someone else has
        made the page that we're running in. Otherwise we're in charge of the
        whole page.

        @return: true if we're running embedded
        """
        return (ApplicationConnection.GENERATED_BODY_CLASSNAME not in
                self.getElement().getOwnerDocument().getBody().getClassName())
    ...
    def updateFromUIDL(self, uidl, client):
        ...
        if uidl.hasAttribute('style'):
            self.setStyleName(self.getStylePrimaryName() + ' '
        ...
        if not self.isEmbedded():
            ...
            muntjac.gwt.user.client.Window.setTitle(
        ...
        # Open URL:s
        isClosed = False  # was this window closed?
        while (childIndex < uidl.getChildCount()
                and 'open' == uidl.getChildUIDL(childIndex).getTag()):
        ...
        if uidl.hasAttribute('focused'):
            ...
            class _2_(Command):
                ...
                def execute(self):
                    toBeFocused = self.uidl.getPaintableAttribute('focused',
                            VView_this._connection)
                    # Two types of Widgets can be focused, either implementing
                    # GWT HasFocus of a thinner Vaadin specific Focusable
                    # interface.
                    if isinstance(toBeFocused, gwt.user.client.ui.Focusable):
        ...
        if uidl.hasVariable('scrollTop'):
            ...
            DOM.setElementPropertyInt(self.getElement(), 'scrollTop',
                    self._scrollTop)
            self._scrollLeft = uidl.getIntVariable('scrollLeft')
            DOM.setElementPropertyInt(self.getElement(), 'scrollLeft',
                    self._scrollLeft)
        else:
    ...
    def scrollIntoView(self, uidl):
        ...
        if uidl.hasAttribute('scrollTo'):
            ...
            class _3_(Command):
                ...
                def execute(self):
                    paintable = self.uidl.getPaintableAttribute('scrollTo',
                            VView_this._connection)
    ...
    def updateScrollPosition(self):
        ...
        if self._connection is not None and not self._rendering:
            if oldTop != self._scrollTop:
                self._connection.updateVariable(self._id, 'scrollTop',
                        self._scrollTop, False)
            if oldLeft != self._scrollLeft:
                self._connection.updateVariable(self._id, 'scrollLeft',
    ...
    def onResize(self, event=None):
        ...
        if event is None:
            ...
            lazy = ((self._resizeLazy
                    or (BrowserInfo.get().isIE() and BrowserInfo.get().getIEVersion() <= 8))
                    or BrowserInfo.get().isFF3())
            if lazy:
                self._delayedResizeExecutor.trigger()
            else:
                self.windowSizeMaybeChanged(Window.getClientWidth(),
                        Window.getClientHeight())
    ...
    def sendClientResized(self):
        ...
        self._connection.updateVariable(self._id, 'height', self._height,
                False)
        self._connection.updateVariable(self._id, 'width', self._width,
                self._immediate)
...
class MyRenderSpace(RenderSpace):
    ...
    def detectExcessSize(self):
        ...
        if (BrowserInfo.get().isIE()
                and self.getElement().getPropertyInt('clientWidth') == 0):
            ...
            self._excessWidth = (self.getElement().getOffsetWidth()
                    - div.getOffsetWidth())
            self.getElement().removeChild(div)
        else:
            self._excessWidth = (self.getElement().getOffsetWidth()
                    - self.getElement().getPropertyInt('clientWidth'))
        self._excessHeight = (self.getElement().getOffsetHeight()
                - self.getElement().getPropertyInt('clientHeight'))
    ...
    def getRealWidth(self):
        ...
        # Find all Muntjac applications on the page
        vaadinApps = list()
        ...
        # Set display: none for all Muntjac apps
        for i in range(len(vaadinApps)):
            appId = vaadinApps[i]
            if appId == ownAppId:
