...
#    };
        ...
        if uidl.hasAttribute('type'):
            ...
            if self._type == 'image':
                ...
                if nodes is not None and nodes.getLength() == 1:
                    n = nodes.getItem(0)
                    if n.getNodeType() == self.Node.ELEMENT_NODE:
                        e = n
                        if e.getTagName() == 'IMG':
                            el = e
                if el is None:
                    self.setHTML('')
                    el = self.DOM.createImg()
                    created = True
                    client.addPngFix(el)
                    self.DOM.sinkEvents(el, self.Event.ONLOAD)
                # Set attributes
                style = el.getStyle()
                ...
                self.DOM.setElementProperty(el, 'src', self.getSrc(uidl, client))
                if created:
                    # insert in dom late
                ...
                if self._browserElement is None:
                    self.setHTML('<iframe width=\"100%\" height=\"100%\" frameborder=\"0\" allowTransparency=\"true\" src=\"' + self.getSrc(uidl, client) + '\" name=\"' + uidl.getId() + '\"></iframe>')
                    self._browserElement = self.DOM.getFirstChild(self.getElement())
                else:
                    self.DOM.setElementAttribute(self._browserElement, 'src', self.getSrc(uidl, client))
            ...
            if mime == 'application/x-shockwave-flash':
                ...
                self.setHTML('')
                obj = self.Document.get().createObjectElement()
                obj.setType(mime)
                obj.setData(data)
                ...
                if uidl.hasAttribute('classid'):
                    obj.setAttribute('classid', uidl.getStringAttribute(self.escapeAttribute('classid')))
                if uidl.hasAttribute('codebase'):
                    obj.setAttribute('codebase', uidl.getStringAttribute(self.escapeAttribute('codebase')))
                if uidl.hasAttribute('codetype'):
                    obj.setAttribute('codetype', uidl.getStringAttribute(self.escapeAttribute('codetype')))
                if uidl.hasAttribute('archive'):
                    obj.setAttribute('archive', uidl.getStringAttribute(self.escapeAttribute('archive')))
                if uidl.hasAttribute('standby'):
                    obj.setAttribute('standby', uidl.getStringAttribute(self.escapeAttribute('standby')))
    ...
    def createFlashEmbed(self, uidl):
        ...
        if uidl.hasAttribute('classid'):
            html.append('classid=\"' + self.escapeAttribute(uidl.getStringAttribute('classid')) + '\" ')
        else:
        ...
        if uidl.hasAttribute('codebase'):
            html.append('codebase=\"' + self.escapeAttribute(uidl.getStringAttribute('codebase')) + '\" ')
        else:
            html.append('codebase=\"http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,0,0\" ')
        # Add width and height
        html.append('width=\"' + self._width + '\" ')
        html.append('height=\"' + self._height + '\" ')
        ...
        for name in parameters.keys():
            html.append('<param ')
            html.append('name=\"' + self.escapeAttribute(name) + '\" ')
            html.append('value=\"' + self.escapeAttribute(parameters[name]) + '\" ')
            html.append('/>')
        # Build inner EMBED tag
        html.append('<embed ')
        html.append('src=\"' + self.getSrc(uidl, self._client) + '\" ')
        html.append('width=\"' + self._width + '\" ')
        html.append('height=\"' + self._height + '\" ')
        html.append('type=\"application/x-shockwave-flash\" ')
        # Add the parameters to the Embed
        for name in parameters.keys():
            html.append(self.escapeAttribute(name))
            html.append('=')
            html.append('\"' + self.escapeAttribute(parameters[name]) + '\"')
        # End embed tag
        html.append('></embed>')
        # End object tag
        html.append('</object>')
    ...
    def escapeAttribute(self, attribute):
    ...
    def onDetach(self):
        if BrowserInfo.get().isIE():
            ...
            if self._browserElement is not None:
                self.DOM.setElementAttribute(self._browserElement, 'src', 'javascript:false')
    ...
    def updateElementDynamicSizeFromImage(self):
        ...
        if self.isDynamicWidth():
            self.getElement().getStyle().setWidth(self.getElement().getFirstChildElement().getOffsetWidth(), self.Unit.PX)
        if self.isDynamicHeight():
            self.getElement().getStyle().setHeight(self.getElement().getFirstChildElement().getOffsetHeight(), self.Unit.PX)
