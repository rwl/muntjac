...
class ShortcutActionHandler(object):
    ...
    def handleKeyboardEvent(self, *args):
        ...
        if _1 == 1:
            ...
            keyCode = self.DOM.eventGetKeyCode(event)
    ...
    def fireAction(self, event, a, target):
        et = self.DOM.eventGetTarget(event)
        if target is None:
        ...
        if isinstance(finalTarget, BeforeShortcutActionListener):
        ...
        class _1_(Command):
            ...
            def execute(self):
                if self.finalTarget is not None:
                    self.client.updateVariable(self.paintableId, 'actiontarget', self.finalTarget, False)
                self.client.updateVariable(self.paintableId, 'action', self.a.getKey(), True)
