...
class VMenuBar(SimpleFocusablePanel, Paintable, CloseHandler, ContainerResizedListener, KeyPressHandler, KeyDownHandler, FocusHandler, SubPartAware):
    ...
    def __init__(self, *args):
        ...
        if _1 == 0:
            ...
            self.sinkEvents(((Event.ONCLICK | Event.ONMOUSEOVER) | Event.ONMOUSEOUT) | Event.ONLOAD)
    ...
    def setWidth(self, width):
        ...
        if BrowserInfo.get().isIE6() and width.endswith('px'):
            ...
            if w < 0:
                w = 0
            self.getElement().getStyle().setWidth(w, Unit.PX)
        else:
    ...
    def updateFromUIDL(self, uidl, client):
        ...
        if uidl.hasAttribute('width'):
            ...
            if moreItemUIDL.hasAttribute('icon'):
                itemHTML.__add__('<img src=\"' + Util.escapeAttribute(client.translateVaadinUri(moreItemUIDL.getStringAttribute('icon'))) + '\" class=\"' + Icon.CLASSNAME + '\" alt=\"\" />')
            moreItemText = moreItemUIDL.getStringAttribute('text')
            if '' == moreItemText:
                moreItemText = '&#x25BA;'
            itemHTML.__add__(moreItemText)
            self.moreItem = GWT.create(self.CustomMenuItem)
        ...
        if item.hasAttribute('separator'):
            ...
            if item.getChildCount() > 0:
                ...
                if self.submenuIcon is not None:
                    bgStyle = ' style=\"background-image: url(' + Util.escapeAttribute(self.submenuIcon) + '); text-indent: -999px; width: 1em;\"'
            ...
            if item.hasAttribute('icon'):
                itemHTML.__add__('<img src=\"' + Util.escapeAttribute(self.client.translateVaadinUri(item.getStringAttribute('icon'))) + '\" class=\"' + Icon.CLASSNAME + '\" alt=\"\" />')
            itemText = item.getStringAttribute('text')
            if not self._htmlContentAllowed:
                itemText = Util.escapeHTML(itemText)
            itemHTML.__add__(itemText)
    ...
    def addItem(self, *args):
        ...
        if _1 == 1:
            ...
            if isinstance(_0[0], self.CustomMenuItem):
                item, index = _0
                if self.items.contains(item):
                    return
                DOM.insertChild(self.getContainerElement(), item.getElement(), index)
                item.setParentMenu(self)
                item.setSelected(False)
                self.items.add(index, item)
            else:
                html, cmd = _0
                item = GWT.create(self.CustomMenuItem)
                item.setHTML(html)
                item.setCommand(cmd)
                self.addItem(item)
                return item
    ...
    def removeItem(self, item):
        ...
        if self.items.contains(item):
            ...
            DOM.removeChild(self.getContainerElement(), DOM.getChild(self.getContainerElement(), index))
        ...
        if targetItem is not None:
            ...
            while True:
                if _1 == Event.ONCLICK:
                    ...
                    break
                if (_2 is True) or (_1 == Event.ONMOUSEOVER):
                    _2 = True
                    self.LazyCloser.cancelClosing()
                    if self.isEnabled() and targetItem.isEnabled():
                        self.itemOver(targetItem)
                    break
                if (_2 is True) or (_1 == Event.ONMOUSEOUT):
                    _2 = True
                    self.itemOut(targetItem)
                    self.LazyCloser.schedule()
                    break
                break
        elif self.subMenu and DOM.eventGetType(e) == Event.ONCLICK and self.subMenu:
            # Prevent moving keyboard focus to child menus
            parent = self.parentMenu
            while parent.getParentMenu() is not None:
                parent = parent.getParentMenu()
    ...
    def LazyCloser(VMenuBar_this, *args, **kwargs):
        ...
        class LazyCloser(Timer):
            ...
            def prepare(cls, vMenuBar):
                if cls.INSTANCE is None:
                    cls.INSTANCE = VMenuBar_this.LazyCloser()
            ...
            class _1_(Command):
                ...
                def execute(self):
                    if VMenuBar_this.popup is None:
                        ...
                        return
                    if (
                        (VMenuBar_this.popup.getElement().getStyle().getProperty('width') is None) or (VMenuBar_this.popup.getElement().getStyle().getProperty('width') == '')
                    ):
    ...
    def adjustPopupHeight(self, top, shadowSpace):
        ...
        availableHeight = RootPanel.getBodyElement().getOffsetHeight() - top - shadowSpace
        ...
        if missingHeight > 0:
            ...
            moveUpBy = self.Math.min(top - shadowSpace, missingHeight)
            # Update state
            top -= moveUpBy
    ...
    def CustomMenuItem(VMenuBar_this, *args, **kwargs):
        ...
        class CustomMenuItem(Widget, HasHTML):
            ...
            def __init__(self, *args):
                ...
                if _1 == 0:
                    ...
                    self.setStyleName(VMenuBar_this.CLASSNAME + '-menuitem')
                    self.sinkEvents(VTooltip.TOOLTIP_EVENTS)
                else:
                    raise ARGERROR(0, 2)
            ...
            def setSeparator(self, separator):
                ...
                if separator:
                    self.setStyleName(VMenuBar_this.CLASSNAME + '-separator')
                else:
                    self.setStyleName(VMenuBar_this.CLASSNAME + '-menuitem')
            ...
            def updateFromUIDL(self, uidl, client):
                ...
                if (
                    not self.isSeparator() and uidl.hasAttribute(VMenuBar_this.ATTRIBUTE_CHECKED)
                ):
                    # if the selected attribute is present (either true or false),
                    # the item is selectable
                    self.setCheckable(True)
                    self.setChecked(uidl.getBooleanAttribute(VMenuBar_this.ATTRIBUTE_CHECKED))
                else:
        ...
        return CustomMenuItem(*args, **kwargs)
