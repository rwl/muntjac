...
class VFormLayout(SimplePanel, Container):
    ...
    class VFormLayoutTable(FlexTable, ClickHandler):
        ...
        def __init__(self):
            self.DOM.setElementProperty(self.getElement(), 'cellPadding', '0')
            self.DOM.setElementProperty(self.getElement(), 'cellSpacing', '0')

        def updateFromUIDL(self, uidl, client):
            margins = VMarginInfo(uidl.getIntAttribute('margins'))
            margin = self.getElement()
            self.setStyleName(margin, self.CLASSNAME + '-' + StyleConstants.MARGIN_TOP, margins.hasTop())
            self.setStyleName(margin, self.CLASSNAME + '-' + StyleConstants.MARGIN_RIGHT, margins.hasRight())
            self.setStyleName(margin, self.CLASSNAME + '-' + StyleConstants.MARGIN_BOTTOM, margins.hasBottom())
            self.setStyleName(margin, self.CLASSNAME + '-' + StyleConstants.MARGIN_LEFT, margins.hasLeft())
            self.setStyleName(margin, self.CLASSNAME + '-' + 'spacing', uidl.hasAttribute('spacing'))
            i = 0
            _0 = True
            ...
            while True:
                ...
                if oldComponent is None:
                    ...
                    self.setWidget(i, self._COLUMN_WIDGET, p)
                self.getCellFormatter().setStyleName(i, self._COLUMN_WIDGET, self.CLASSNAME + '-contentcell')
                self.getCellFormatter().setStyleName(i, self._COLUMN_CAPTION, self.CLASSNAME + '-captioncell')
                self.setWidget(i, self._COLUMN_CAPTION, caption)
                self.setContentWidth(i)
                self.getCellFormatter().setStyleName(i, self._COLUMN_ERRORFLAG, self.CLASSNAME + '-errorcell')
                self.setWidget(i, self._COLUMN_ERRORFLAG, error)
                p.updateFromUIDL(childUidl, client)
                rowstyles = self.CLASSNAME + '-row'
                if i == 0:
                    rowstyles += ' ' + self.CLASSNAME + '-firstrow'
                if not it.hasNext():
                    rowstyles += ' ' + self.CLASSNAME + '-lastrow'
        ...
        def replaceChildComponent(self, oldComponent, newComponent):
            ...
            while True:
                ...
                if oldComponent == candidate:
                    ...
                    newCap = self.Caption(newComponent, self.client, None)
        ...
        def getAllocatedWidth(self, child, availableWidth):
            ...
            if caption is not None:
                width -= self.DOM.getParent(caption.getElement()).getOffsetWidth()
            if error is not None:
                width -= self.DOM.getParent(error.getElement()).getOffsetWidth()
            return width
        ...
        def onClick(self, event):
            ...
            if caption.getOwner() is not None:
                ...
                elif isinstance(caption.getOwner(), com.google.gwt.user.client.ui.Focusable):
    ...
    class Caption(HTML):
        ...
        def updateCaption(self, uidl):
            ...
            self.setStyleName(self.getElement(), 'v-disabled', uidl.hasAttribute('disabled'))
            isEmpty = True
            if uidl.hasAttribute('icon'):
                if self._icon is None:
                    self._icon = Icon(self._client)
                    self.DOM.insertChild(self.getElement(), self._icon.getElement(), 0)
            ...
            if uidl.hasAttribute('caption'):
                if self._captionText is None:
                    self._captionText = self.DOM.createSpan()
                    self.DOM.insertChild(self.getElement(), self._captionText, 0 if self._icon is None else 1)
            ...
            if uidl.getBooleanAttribute('required'):
                if self._requiredFieldIndicator is None:
                    self._requiredFieldIndicator = self.DOM.createSpan()
                    self.DOM.setInnerText(self._requiredFieldIndicator, '*')
                    self.DOM.setElementProperty(self._requiredFieldIndicator, 'className', 'v-required-field-indicator')
                    self.DOM.appendChild(self.getElement(), self._requiredFieldIndicator)
            ...
            if BrowserInfo.get().isIE():
                if isEmpty:
                    ...
                    self.setHeight('')
                    self.DOM.setStyleAttribute(self.getElement(), 'overflow', '')
    ...
    class ErrorFlag(HTML):
        ...
        def updateFromUIDL(self, uidl, component):
            ...
            if uidl.hasAttribute('error') and not uidl.getBooleanAttribute('hideErrors'):
                ...
                self.DOM.removeChild(self.getElement(), self._errorIndicatorElement)
                self._errorIndicatorElement = None

        def onBrowserEvent(self, event):
            super(ErrorFlag, self).onBrowserEvent(event)
            if self._owner is not None:
                self.client.handleTooltipEvent(event, self._owner)
