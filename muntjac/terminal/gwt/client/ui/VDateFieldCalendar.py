...
# from com.vaadin.terminal.gwt.client.ui.VCalendarPanel.FocusChangeListener import (FocusChangeListener,)
# from com.vaadin.terminal.gwt.client.ui.VCalendarPanel.FocusOutListener import (FocusOutListener,)
# from com.vaadin.terminal.gwt.client.ui.VCalendarPanel.SubmitListener import (SubmitListener,)
# from com.vaadin.terminal.gwt.client.ui.VCalendarPanel.TimeChangeListener import (TimeChangeListener,)
...
class VDateFieldCalendar(VDateField):
    ...
    def __init__(self):
        super(VDateFieldCalendar, self)()
        self._calendarPanel = VCalendarPanel()
        self.add(self._calendarPanel)

        class _0_(SubmitListener):

            def onSubmit(self):
                self.updateValueFromPanel()
            ...
            class _2_(TimeChangeListener):
                ...
                def changed(self, hour, min, sec, msec):
                    ...
                    if d is None:
                        ...
                        d = self.calendarPanel.getDate().clone()
                    d.setHours(hour)
                    d.setMinutes(min)
                    d.setSeconds(sec)
                    DateTimeService.setMilliseconds(d, msec)
                    # Always update time changes to the server
                    self.calendarPanel.setDate(d)
                    self.updateValueFromPanel()
        ...
        if self.currentResolution <= self.RESOLUTION_MONTH:
            ...
            class _3_(FocusChangeListener):
                ...
                def focusChanged(self, date):
                    date2 = Date()
                    if self.calendarPanel.getDate() is not None:
                        date2.setTime(self.calendarPanel.getDate().getTime())
                    # Update the value of calendarPanel
                    date2.setYear(date.getYear())
                    date2.setMonth(date.getMonth())
                    self.calendarPanel.setDate(date2)
    ...
    def updateValueFromPanel(self):
        ...
        if (currentDate is None) or (date2.getTime() != currentDate.getTime()):
            ...
            self.getClient().updateVariable(self.getId(), 'year', date2.getYear() + 1900, False)
            if self.getCurrentResolution() > VDateField.RESOLUTION_YEAR:
                self.getClient().updateVariable(self.getId(), 'month', date2.getMonth() + 1, False)
                if self.getCurrentResolution() > self.RESOLUTION_MONTH:
                    self.getClient().updateVariable(self.getId(), 'day', date2.getDate(), False)
                    if self.getCurrentResolution() > self.RESOLUTION_DAY:
                        self.getClient().updateVariable(self.getId(), 'hour', date2.getHours(), False)
                        if self.getCurrentResolution() > self.RESOLUTION_HOUR:
                            self.getClient().updateVariable(self.getId(), 'min', date2.getMinutes(), False)
                            if self.getCurrentResolution() > self.RESOLUTION_MIN:
                                self.getClient().updateVariable(self.getId(), 'sec', date2.getSeconds(), False)
                                if self.getCurrentResolution() > self.RESOLUTION_SEC:
                                    self.getClient().updateVariable(self.getId(), 'msec', DateTimeService.getMilliseconds(date2), False)
            if self.isImmediate():
