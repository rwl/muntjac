...
class VTablePaging(Composite, Table, Paintable, ClickHandler):
    ...
    def onClick(self, event):
        ...
        if isinstance(sender, HeaderCell):
            hCell = sender
    ...
    class HeaderCell(HTML):
        ...
        def __init__(self, pid, caption):
            ...
            self._cid = pid
            self.addClickHandler(_VTablePaging_this)
            self.setText(caption)
            # TODO remove debug color
            self.DOM.setStyleAttribute(self.getElement(), 'color', 'brown')
            self.DOM.setStyleAttribute(self.getElement(), 'font-weight', 'bold')
    ...
    class BodyCell(SimplePanel):
        ...
        def __init__(self, *args):
            ...
            if _1 == 1:
                row, = _0
                super(BodyCell, self)()
                self.sinkEvents(self.Event.BUTTON_LEFT | self.Event.BUTTON_RIGHT)
                self._row = row
            elif _1 == 2:
                row2, textContent = _0
                super(BodyCell, self)()
                self.sinkEvents(self.Event.BUTTON_LEFT | self.Event.BUTTON_RIGHT)
                self._row = row2
                self.setWidget(self.Label(textContent))
            else:
                raise ARGERROR(1, 2)
        ...
        def onBrowserEvent(self, event):
            ...
            _1 = False
            while True:
                if _0 == self.Event.BUTTON_RIGHT:
                    _1 = True
                    self._row.showContextMenu(event)
                    self.Window.alert('context menu un-implemented')
                    self.DOM.eventCancelBubble(event, True)
                    break
                if (_1 is True) or (_0 == self.Event.BUTTON_LEFT):
                    _1 = True
                    if self.selectMode > Table.SELECT_MODE_NONE:
    ...
    class TableRow(object):
        ...
        def __init__(self, rowIndex, rowKey, selected):
            self.rowKeysToTableRows.put(rowKey, self)
        ...
        def setSelected(self, sel):
            ...
            self._selected = sel
            if self._selected:
                self.selectedRowKeys.add(self._key)
                self.DOM.setStyleAttribute(self.tBody.getRowFormatter().getElement(self._rowIndex), 'background', 'yellow')
            else:
                self.selectedRowKeys.remove(self._key)
                self.DOM.setStyleAttribute(self.tBody.getRowFormatter().getElement(self._rowIndex), 'background', 'transparent')
        ...
        def toggleSelected(self):
            ...
            if self._selected:
                ...
                if self.selectMode == Table.SELECT_MODE_SINGLE:
                    self.deselectAll()
                self.setSelected(True)
            self.client.updateVariable(self.id, 'selected', list([None] * len(self.selectedRowKeys)), self.immediate)
