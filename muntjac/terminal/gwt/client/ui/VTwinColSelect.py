...
class VTwinColSelect(VOptionGroupBase, KeyDownHandler, MouseDownHandler, DoubleClickHandler, SubPartAware):
    ...
    def __init__(self):
        ...
        self._captionWrapper.getElement().getStyle().setOverflow(Overflow.HIDDEN)
    ...
    def onKeyDown(self, event):
        ...
        if keycode == KeyCodes.KEY_TAB and event.getSource() == self._options:
        ...
        if (
            keycode == KeyCodes.KEY_TAB and event.isShiftKeyDown() and event.getSource() == self._selections
        ):
            # Prevent default behavior
            event.preventDefault()
            # Remove current selections
