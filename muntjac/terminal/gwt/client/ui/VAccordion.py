...
class VAccordion(VTabsheetBase, ContainerResizedListener):
    ...
    def __init__(self):
        ...
        if BrowserInfo.get().isIE6():
            self.DOM.setStyleAttribute(self.getElement(), 'zoom', '1')
    ...
    class StackItem(ComplexPanel, ClickHandler):
        ...
        def setHeight(self, height):
            if height == -1:
                ...
                self.DOM.setStyleAttribute(self._content, 'height', '0px')
            else:
                super(StackItem, self).setHeight(height + self.getCaptionHeight() + 'px')
                self.DOM.setStyleAttribute(self._content, 'height', height + 'px')
                self.DOM.setStyleAttribute(self._content, 'top', self.getCaptionHeight() + 'px')
        ...
        def __init__(self, tabUidl):
            ...
            if BrowserInfo.get().isIE6():
                self.DOM.setEventListener(self._captionNode, self)
                self.DOM.sinkEvents(self._captionNode, self.Event.BUTTON_LEFT)
            super(StackItem, self).add(self._caption, self._captionNode)
            self.DOM.appendChild(self._captionNode, self._caption.getElement())
            self.DOM.appendChild(self.getElement(), self._captionNode)
            self.DOM.appendChild(self.getElement(), self._content)
            self.setStyleName(self.CLASSNAME + '-item')
            self.DOM.setElementProperty(self._content, 'className', self.CLASSNAME + '-item-content')
            self.DOM.setElementProperty(self._captionNode, 'className', self.CLASSNAME + '-item-caption')
            self.close()

        def onBrowserEvent(self, event):
            self.onSelectTab(self)
        ...
        def replacePaintable(self, newPntbl):
            if self.getWidgetCount() > 1:
                ...
                self.paintables.remove(self.getWidget(1))
                self.remove(1)
            self.add(newPntbl, self._content)
            self.paintables.add(newPntbl)

        def open(self):
            self._open = True
            self.DOM.setStyleAttribute(self._content, 'top', self.getCaptionHeight() + 'px')
            self.DOM.setStyleAttribute(self._content, 'left', '0px')
            self.DOM.setStyleAttribute(self._content, 'visibility', '')
            self.addStyleDependentName('open')

        def hide(self):
            self.DOM.setStyleAttribute(self._content, 'visibility', 'hidden')

        def close(self):
            self.DOM.setStyleAttribute(self._content, 'visibility', 'hidden')
            self.DOM.setStyleAttribute(self._content, 'top', '-100000px')
            self.DOM.setStyleAttribute(self._content, 'left', '-100000px')
        ...
        def setContent(self, contentUidl):
            ...
            if self.isOpen() and self.isDynamicHeight():
        ...
        def getWidgetWidth(self):
            return self.DOM.getFirstChild(self._content).getOffsetWidth()
