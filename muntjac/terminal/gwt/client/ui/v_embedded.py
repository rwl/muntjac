...
class VEmbedded(HTML, IPaintable):
    ...
    def updateFromUIDL(self, uidl, client):
        ...
        if uidl.hasAttribute('type'):
            ...
            if self._type == 'image':
                ...
                DOM.setElemAttribute(el, 'src', self.getSrc(uidl, client))
                ...
                if self._browserElement is None:
                    self.setHTML('<iframe width=\"100%\" height=\"100%\" frameborder=\"0\"'
                            + ' allowTransparency=\"true\" src=\"\"'
                            + ' name=\"' + uidl.getId() + '\"></iframe>')
                    self._browserElement = DOM.getFirstChild(self.getElement())
                DOM.setElemAttribute(self._browserElement, 'src',
                        self.getSrc(uidl, client))
    ...
    def createFlashEmbed(self, uidl):
        ...
        html.append('<embed ')
        html.append('src=\"'
                + Util.escapeAttribute(self.getSrc(uidl, self._client))
                + '\" ')
        html.append('width=\"' + Util.escapeAttribute(self._width) + '\" ')
        html.append('height=\"' + Util.escapeAttribute(self._height) + '\" ')
        html.append('type=\"application/x-shockwave-flash\" ')
    ...
    def setWidth(self, width):
        ...
        if self.isDynamicHeight():
            ...
            # Must notify parent if the height changes as a result of
            # a width change
    ...
    def updateElementDynamicSizeFromImage(self):
        ...
        if self.isDynamicWidth():
            self.getElement().getStyle().setWidth(self.getElement().getFirstChildElement().getOffsetWidth(), 'px')
        if self.isDynamicHeight():
            self.getElement().getStyle().setHeight(self.getElement().getFirstChildElement().getOffsetHeight(), 'px')
