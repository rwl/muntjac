...
from muntjac.terminal.gwt.client.ui.shortcut_action_handler \
    import ShortcutActionHandlerOwner
...
class VPanel(SimplePanel, Container, ShortcutActionHandlerOwner, IFocusable):
    ...
    def __init__(self):
        ...
        self._icon = None
        self._bottomDecoration = DOM.createDiv()
        ...
        self._width = ''
        self._geckoCaptionMeter = None
        self._scrollTop = None
        self._scrollLeft = None
        self._renderInformation = RenderInformation()
        self._borderPaddingHorizontal = -1
        self._borderPaddingVertical = -1
        self._captionPaddingHorizontal = -1
        self._captionMarginLeft = -1
        self._rendering = None
        self._contentMarginLeft = -1
        self._previousStyleName = None
        # self.clickEventHandler = new ClickEventHandler(this,
        ...
        captionWrap = DOM.createDivElement()
    ...
    def updateFromUIDL(self, uidl, client):
        ...
        if not uidl.hasAttribute('cached'):
            ...
            if (uidl.hasAttribute('caption')
                    and not (uidl.getStringAttribute('caption') == '')):
            ...
            if uidl.hasAttribute('style'):
                ...
                captionBaseClass = (self.CLASSNAME
                        + ('-caption' if hasCaption else '-nocaption'))
            ...
            for i in range(1, cnt):
                ...
                if childUidl.getTag() == 'actions':
                    if self._shortcutHandler is None:
                        self._shortcutHandler = ShortcutActionHandler(self._id,
                                client)
                    self._shortcutHandler.updateActionMap(childUidl)

        if (uidl.hasVariable('scrollTop')
                and uidl.getIntVariable('scrollTop') != self._scrollTop):
        ...
        if (uidl.hasVariable('scrollLeft')
                and uidl.getIntVariable('scrollLeft') != self._scrollLeft):
    ...
    def handleError(self, uidl):
        if uidl.hasAttribute('error'):
            ...
            DOM.insertBefore(self._captionNode, self._errorIndicatorElement,
                    self._captionText)
    ...
    def runHacks(self, runGeckoFix):
        if (BrowserInfo.get().isIE6() and self._width is not None
                and (self._width != '')):
            ...
            parentPadding = Util.measureHorizontalPaddingAndBorder(
                    self.getElement(), 0)

            parentWidthExcludingPadding = (self.getElement().getOffsetWidth()
                    - parentPadding)

            Util.setWidthExcludingPaddingAndBorder(self._captionNode,
                    parentWidthExcludingPadding - self.getCaptionMarginLeft(),
                    26, False)

            contentMarginLeft = self.getContentMarginLeft()

            Util.setWidthExcludingPaddingAndBorder(self._contentNode,
                    parentWidthExcludingPadding - contentMarginLeft, 2, False)

        if ((BrowserInfo.get().isIE() or BrowserInfo.get().isFF2())
                and ((self._width is None) or (self._width == ''))):

            # IE and FF2 needs width to be specified for the root DIV so we
            # calculate that from the sizes of the caption and layout
            captionWidth = (self._captionText.getOffsetWidth()
                    + self.getCaptionMarginLeft()
                    + self.getCaptionPaddingHorizontal())

            layoutWidth = (self._layout.getOffsetWidth()
                    + self.getContainerBorderWidth())
            width = layoutWidth
            ...
            if BrowserInfo.get().isIE7():
                Util.setWidthExcludingPaddingAndBorder(self._captionNode,
                        width - self.getCaptionMarginLeft(), 26, False)
        ...
        if runGeckoFix and BrowserInfo.get().isGecko():
            ...
            if (width is None) or (width == ''):
                ...
                captionWidth = DOM.getIntElemAttribute(self._captionText,
                        'offsetWidth')
                availWidth = DOM.getIntElemAttribute(self._geckoCaptionMeter,
                        'offsetWidth')
    ...
    def onBrowserEvent(self, event):
        ...
        if type == Event.ONSCROLL:
            newscrollTop = DOM.getIntElemAttribute(self._contentNode,
                    'scrollTop')
            newscrollLeft = DOM.getIntElemAttribute(self._contentNode,
                    'scrollLeft')
            if (self._client is not None
                    and (newscrollLeft != self._scrollLeft)
                    or (newscrollTop != self._scrollTop)):
                self._scrollLeft = newscrollLeft
                self._scrollTop = newscrollTop
                self._client.updateVariable(self._id, 'scrollTop',
                        self._scrollTop, False)
                self._client.updateVariable(self._id, 'scrollLeft',
                        self._scrollLeft, False)
    ...
    def setHeight(self, height):
        ...
        if height is not None and ('' != height):
            ...
            containerHeight = (targetHeight
                    - self._captionNode.getParentElement().getOffsetHeight()
                    - self._bottomDecoration.getOffsetHeight()
                    - self.getContainerBorderHeight())
    ...
    def setWidth(self, width):
        ...
        if not self._rendering:
            ...
            if self._height == '':
                ...
                Util.updateRelativeChildrenAndSendSizeUpdateEvent(self._client,
                        self)
    ...
    def detectContainerBorders(self):
        ...
        self._borderPaddingHorizontal = Util.measureHorizontalBorder(
                self._contentNode)
        self._borderPaddingVertical = Util.measureVerticalBorder(
                self._contentNode)
        DOM.setStyleAttribute(self._contentNode, 'overflow', 'auto')
        self._captionPaddingHorizontal = Util.measureHorizontalPaddingAndBorder(
                self._captionNode, 26)
    ...
    def requestLayout(self, child):
        ...
        if (self._height is not None and self._height != ''
                and self._width is not None and self._width != ''):
