...
class VContextMenu(VOverlay, SubPartAware):
    ...
    def showAt(self, *args):
        ...
        if _1 == 2:
            ...
            _VContextMenu_this = self

            class _3_(PositionCallback):

                def setPosition(self, offsetWidth, offsetHeight):
                    # mac FF gets bad width due GWT popups overflow hacks,
                    # re-determine width
                    offsetWidth = self.menu.getOffsetWidth()
                    left = _VContextMenu_this.left
                    top = _VContextMenu_this.top
                    ...
                    class _2_(Command):
                        ...
                        def execute(self):
                            # Focus the menu.
                            self.menu.setFocus(True)
                            # Unselect previously selected items
                            self.menu.selectItem(None)

                    _2_ = self._2_()
                    self.Scheduler.get().scheduleDeferred(_2_)

            _3_ = self._3_()
            self.setPopupPositionAndShow(_3_)
        elif _1 == 3:
            ao, left, top = _0
    ...
    class CMenuBar(MenuBar, HasFocusHandlers, HasBlurHandlers, HasKeyDownHandlers, HasKeyPressHandlers, Focusable, LoadHandler):
        ...
        def onLoad(self, event):
            ...
            if BrowserInfo.get().isIE6():
                ...
                Util.doIE6PngFix(self.Element.as_(event.getNativeEvent().getEventTarget()))
            self.delayedImageLoadExecutioner.trigger()
