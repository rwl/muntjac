...
class VTabsheetPanel(ComplexPanel):
    ...
    def __init__(self):
        ...
        class _0_(TouchStartHandler):
            ...
            def onTouchStart(self, event):
                ...
                VTabsheetPanel_this.getTouchScrollDelegate().setElements(elements)
                VTabsheetPanel_this.getTouchScrollDelegate().onTouchStart(event)

        _0_ = _0_()
        self.addDomHandler(_0_, TouchStartEvent.getType())
    ...
    def add(self, w):
        ...
        DOM.appendChild(self.getElement(), el)
    ...
    def insert(self, w, beforeIndex):
        ...
        """
        el = self.createContainerElement()
        DOM.insertChild(self.getElement(), el, beforeIndex)
    ...
    def remove(self, w):
        ...
        if removed:
            ...
            if parent is not None:
                DOM.removeChild(self.getElement(), parent)
        return removed
    ...
    def showWidget(self, index):
        ...
        if self._visibleWidget != newVisible:
            if self._visibleWidget is not None:
                self.hide(DOM.getParent(self._visibleWidget.getElement()))
            self._visibleWidget = newVisible
            self.unHide(DOM.getParent(self._visibleWidget.getElement()))
    ...
    def runWebkitOverflowAutoFix(self):
        if self._visibleWidget is not None:
            Util.runWebkitOverflowAutoFix(DOM.getParent(self._visibleWidget.getElement()))
