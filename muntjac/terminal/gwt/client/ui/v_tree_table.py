...
from muntjac.terminal.gwt.client.ui.scroll_table.v_scroll_table_body import VScrollTableBody,\
    VScrollTableRow

from muntjac.terminal.gwt.client.ui.scroll_table.v_scroll_table \
    import VScrollTable

from muntjac.terminal.gwt.client.computed_style import ComputedStyle
...
class VTreeTable(VScrollTable):
    ...
    def __init__(self):
        self._collapseRequest = None
        self._selectionPending = None
        self._colIndexOfHierarchy = None
        self._collapsedRowKey = None
        self._scrollBody = None
        self._animationsEnabled = None
        self._pendingNavigationEvents = list()
        self._focusParentResponsePending = None
    ...
    def updateFromUIDL(self, uidl, client):
        ...
        self._colIndexOfHierarchy = uidl.getIntAttribute(
                self.ATTRIBUTE_HIERARCHY_COLUMN_INDEX) if uidl.hasAttribute(
                        self.ATTRIBUTE_HIERARCHY_COLUMN_INDEX) else 0

        super(VTreeTable, self).updateFromUIDL(uidl, client)
        if self._collapseRequest:
            if (self._collapsedRowKey is not None
        ...
        if uidl.hasAttribute('focusedRow'):
            ...
            # parent
            self._focusParentResponsePending = False
        while (not self._collapseRequest
                and not self._focusParentResponsePending
                and not self._pendingNavigationEvents.isEmpty()):
    ...
    def createScrollBody(self):
        ...
        self._scrollBody = VTreeTableScrollBody()
    ...
    def addAndRemoveRows(self, partialRowAdditions):
        ...
        if self._animationsEnabled and self.browserSupportsAnimation():
            if partialRowAdditions.hasAttribute('hide'):
                self._scrollBody.unlinkRowsAnimatedAndUpdateCacheWhenFinished(
                        partialRowAdditions.getIntAttribute('firstprowix'),
                        partialRowAdditions.getIntAttribute('numprows'))
            else:
                self._scrollBody.insertRowsAnimated(partialRowAdditions,
                        partialRowAdditions.getIntAttribute('firstprowix'),
                        partialRowAdditions.getIntAttribute('numprows'))
    ...
    def buildCaptionHtmlSnippet(self, uidl):
        """Icons rendered into first actual column in TreeTable, not to row
        header cell
        """
    ...
    def handleNavigation(self, keycode, ctrl, shift):
        if self._collapseRequest or self._focusParentResponsePending:
            # Enqueue the event if there might be pending content changes
            # from the server
            if len(self._pendingNavigationEvents) < 10:
                # Only keep 10 keyboard events in the queue
                pendingNavigationEvent = PendingNavigationEvent(keycode,
                        ctrl, shift)
        ...
        if focusedRow is not None:
            if (focusedRow.canHaveChildren
                    and (keycode == KeyboardListener.KEY_RIGHT
                            and not focusedRow.open)
                    or (keycode == KeyboardListener.KEY_LEFT and focusedRow.open)):
                if not ctrl:
                    self.client.updateVariable(self.paintableId,
                            'selectCollapsed', True, False)
                self.sendSelectedRows(False)
                self.sendToggleCollapsedUpdate(focusedRow.getKey())
                return True
            elif keycode == KeyboardListener.KEY_RIGHT and focusedRow.open:
                ...
                if next is not None:
                    if next.depth > focusedRow.depth:
                        ...
                        return super(VTreeTable, self).handleNavigation(
                                self.getNavigationDownKey(), ctrl, shift)
                else:
                ...
                self.client.updateVariable(self.paintableId, 'focusParent',
    ...
    def sendToggleCollapsedUpdate(self, rowKey):
        ...
        self.client.updateVariable(self.paintableId, 'toggleCollapsed',
                rowKey, True)
...
class VTreeTableScrollBody(VScrollTableBody):

    def __init__(self):
        self._identWidth = -1
        super(VTreeTableScrollBody, self).__init__()


    def createRow(self, uidl, aligns2):
        if uidl.hasAttribute('gen_html'):
            # This is a generated row.
            return VTreeTableGeneratedRow(uidl, aligns2)
        return VTreeTableRow(uidl, aligns2)
    ...
    def unlinkRowsAnimatedAndUpdateCacheWhenFinished(self, firstIndex, rows):
        rowsToDelete = list()
        for ix in range(firstIndex, firstIndex + rows):
            row = self.getRowByRowIndex(ix)
            if row is not None:
                rowsToDelete.add(row)

        class anim(RowCollapseAnimation):

            def onComplete(self):
                super(anim, self).onComplete()
    ...
    def insertRowsAnimated(self, rowData, firstIndex, rows):
        ...
        anim = RowExpandAnimation(insertedRows)
        anim.run(150)
        return insertedRows


class VTreeTableRow(VScrollTableRow):

    def __init__(self, uidl, aligns2, tt, ttsb):
        self._tt = tt
        self._ttsb = ttsb

        self._isTreeCellAdded = False
        self._treeSpacer = None
        self._open = None
        self._depth = None
        self._canHaveChildren = None
        self.widgetInHierarchyColumn = None
    ...
    def addCell(self, *args):
        ...
        nargs = len(args)
        if nargs == 5:
            rowUidl, w, align, style, isSorted = args
            super(VTreeTableRow, self).addCell(rowUidl, w, align, style,
                    isSorted)
            if self.addTreeSpacer(rowUidl):
                self.widgetInHierarchyColumn = w
        elif nargs == 7:
            (rowUidl, text, align, style, textIsHTML, isSorted,
                    description) = args
            super(VTreeTableRow, self).addCell(rowUidl, text, align, style,
                    textIsHTML, isSorted, description)
    ...
    def addTreeSpacer(self, rowUidl):
        if self.cellShowsTreeHierarchy(self.getElement().getChildCount() - 1):
            ...
            if rowUidl.hasAttribute('icon'):
                ...
                icon = DOM.createImageElement()
                icon.setClassName('v-icon')
                icon.setAlt('icon')
                icon.setSrc(self.client.translateVaadinUri(
                        rowUidl.getStringAttribute('icon')))
            ...
            self._treeSpacer = DOM.createSpanElement()
    ...
    def cellShowsTreeHierarchy(self, curColIndex):
        if self._isTreeCellAdded:
            return False
        return curColIndex == (self._tt._colIndexOfHierarchy
                + (1 if self.showRowHeaders else 0))


    def onBrowserEvent(self, event):
        if (event.getEventTarget() == self._treeSpacer
                and self._treeSpacer.getClassName().contains('node')):
            if event.getTypeInt() == Event.ONMOUSEUP:
                self._tt.sendToggleCollapsedUpdate(self.getKey())
            return
        super(VTreeTableRow, self).onBrowserEvent(event)


    def setIdent(self):
        if self._tt.getIdentWidth() > 0 and self._depth != 0:
            self._treeSpacer.getStyle().setWidth(
                    (self._depth + 1) * self._tt.getIdentWidth(), 'px')#Unit.PX)


    def onAttach(self):
        super(VTreeTableRow, self).onAttach()
        if self._tt.getIdentWidth() < 0:
            self._tt.detectIdent(self)
    ...
    def getAllocatedSpace(self, child):
        if self.widgetInHierarchyColumn == child:
            ...
            class _0_(RenderSpace):
                ...
                def getWidth(self):
                    return (self.allocatedSpace.getWidth()
                            - self.hierarchyAndIconWidth)
...
class VTreeTableGeneratedRow(VTreeTableRow):
    ...
    def addCellsFromUIDL(self, uidl, aligns, col, visibleColumnIndex):
        ...
        if self._spanColumns:
            ...
            super(VTreeTableGeneratedRow, self).addCellsFromUIDL(uidl, aligns,
                    col, visibleColumnIndex)
    ...
    def addSpannedCell(self, *args):
        ...
        if nargs == 6:
            ...
            td.getStyle().setHeight(self.getRowHeight(), 'px')#Unit.PX)
            if self.addTreeSpacer(rowUidl):
                self.widgetInHierarchyColumn = w
        elif nargs == 8:
    ...
    def calcAndSetSpanWidthOnCell(self, cellIx):
        spanWidth = 0
        for ix in range(ix = 1 if self.showRowHeaders else 0,
                self.tHead.getVisibleCellCount()):
            spanWidth += self.tHead.getHeaderCell(ix).getOffsetWidth()
        Util.setWidthExcludingPaddingAndBorder(
                self.getElement().getChild(cellIx), spanWidth, 13, False)
...
class AnimationPreparator(object):
    ...
    def copyTRBackgroundsToTDs(self, row):
        ...
        for ix in range(tr.getChildCount()):
            td = tr.getChild(ix)
    ...
    def elementHasBackground(self, element):
        ...
        return not (('rgba(0, 0, 0, 0)' == clr.trim())
                or ('transparent' == clr.trim()) or (img is None))
...
class RowExpandAnimation(Animation):
    ...
    def __init__(self, rows):
        self._rows = rows
        self._cloneDiv = None
        self._cloneTable = None
        self.buildAndInsertAnimatingDiv()
        self._preparator = AnimationPreparator(rows[0].getIndex() - 1)
    ...
    def cloneAndAppendRow(self, row):
        ...
        clonedTR.getStyle().setVisibility('visible')#Visibility.VISIBLE)
        self._cloneTable.appendChild(clonedTR)


    def getBaseOffset(self):
        return (self._rows[0].getAbsoluteTop()
                - self._rows[0].getParent().getAbsoluteTop()
                - (len(self._rows) * self.getRowHeight()))
    ...
    def animateDiv(self, progress):
        ...
        self._cloneDiv.getStyle().setTop(self.getBaseOffset() + offset, 'px')#Unit.PX)
    ...
    def setCellWrapperDivsToDisplayNone(self, row):
        tr = row.getElement()
        for ix in range(tr.getChildCount()):
            self.getWrapperDiv(tr, ix).getStyle().setDisplay('none')#Display.NONE)
...
class RowCollapseAnimation(RowExpandAnimation):
    ...
    def calculateDivOffset(self, progress, rh):
        return -super(RowCollapseAnimation, self).calculateDivOffset(progress,
                rh)
