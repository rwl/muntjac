...
class VSplitPanel(ComplexPanel, Container, ContainerResizedListener):
    ...
    class clickEventHandler(ClickEventHandler):
        ...
        def registerHandler(self, handler, type):
            if (
                Event.getEventsSunk(VSplitPanel_this._splitter) & Event.getTypeInt(type.getName()) != 0
            ):
    ...
    def updateSplitPositionToServer(self):
        ...
        pos = 0
        if self._position.find('%') > 0:
            pos = self.Math.round(Float.valueOf.valueOf(self._position[:len(self._position) - 1]))
        else:
