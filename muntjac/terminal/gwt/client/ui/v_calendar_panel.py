...
from muntjac.terminal.gwt.client.ui.v_native_select import VNativeSelect
from muntjac.terminal.gwt.client.ui.focusable_flex_table import FocusableFlexTable
...
class ISubmitListener(object):

    def onSubmit(self):
        """Called when calendar user triggers a submitting operation in
        calendar panel. Eg. clicking on day or hitting enter.
        """
...
class IFocusOutListener(object):
    """Blur listener that listens to blur event from the panel"""

    def onFocusOut(self, event):
        """@return: true if the calendar panel is not used after focus moves
        out"""
        raise NotImplementedError


class IFocusChangeListener(object):
...
class ITimeChangeListener(object):
...
class VEventButton(Button):
    ...
    def __init__(self, cp):
        self.addMouseDownHandler(cp)
        self.addMouseOutHandler(cp)
        self.addMouseUpHandler(cp)


class VCalendarPanel(FocusableFlexTable, ISubPartAware):
#    KeyDownHandler, KeyPressHandler, MouseOutHandler, MouseDownHandler, MouseUpHandler, BlurHandler, FocusHandler):
    ...
    def __init__(self):
        # Represents a click handler for when a user selects a value by using
        # the mouse
        ...
        self._time = None
        self._days = FlexTable()
        self._resolution = VDateField.RESOLUTION_YEAR
        self._focusedRow = None
        self._mouseTimer = None
        self._value = None
        self._enabled = True
        self._readonly = False
        self._dateTimeService = None
        self._showISOWeekNumbers = None
        self._focusedDate = None
        self._selectedDay = None
        self._focusedDay = None
        self._focusOutListener = None
        self._submitListener = None
        self._focusChangeListener = None
        self._timeChangeListener = None
            ...
            if day > 0 and self._focusedDate is not None:
                ...
                for i in range(rowCount):
                    ...
                    for j in range(cellCount):
                        ...
                        if widget is not None and isinstance(widget, Day):
                            curday = widget
    ...
    def buildCalendarHeader(self, needsMonth):
        ...
        """
        self.getRowFormatter().addStyleName(0,
        ...
        if self._prevMonth is None and needsMonth:
            ...
            self.getFlexCellFormatter().setStyleName(0, 3,
                    VDateField.CLASSNAME + '-calendarpanel-nextmonth')
            self.getFlexCellFormatter().setStyleName(0, 1,
        ...
        if self._prevYear is None:
            ...
            self.getFlexCellFormatter().setStyleName(0, 0,
                    VDateField.CLASSNAME + '-calendarpanel-prevyear')
            self.getFlexCellFormatter().setStyleName(0, 4,
        ...
        self.getFlexCellFormatter().setStyleName(0, 2,
                VDateField.CLASSNAME + '-calendarpanel-month')
        self.setHTML(0, 2, '<span class=\"' + VDateField.CLASSNAME
    ...
    def buildCalendarBody(self):
        ...
        self.getFlexCellFormatter().setStyleName(1, 0,
        ...
        self._days.getFlexCellFormatter().setVisible(headerRow, weekColumn,
                self.isShowISOWeekNumbers())

        self._days.getRowFormatter().setStyleName(headerRow,
                VDateField.CLASSNAME + '-calendarpanel-weekdays')

        if self.isShowISOWeekNumbers():
            self._days.getFlexCellFormatter().setStyleName(headerRow,
                    weekColumn, 'v-first')
            self._days.getFlexCellFormatter().setStyleName(headerRow,
                    firstWeekdayColumn, '')
            self._days.getRowFormatter().addStyleName(headerRow,
        ...
        for i in range(7):
            ...
            if self.getResolution() > VDateField.RESOLUTION_MONTH:
                self._days.setHTML(headerRow, firstWeekdayColumn + i,
                        '<strong>' + self.getDateTimeService().getShortDay(day)
                        + '</strong>')
            else:
                self._days.setHTML(headerRow, firstWeekdayColumn + i, '')
        ...
        if (not initiallyNull
                and self._value.getMonth() == self._focusedDate.getMonth()
                and self._value.getYear() == self._focusedDate.getYear()):
            dayOfMonthSelected = self._value.getDate()

        today = Date()
        if (today.getMonth() == self._focusedDate.getMonth()
                and today.getYear() == self._focusedDate.getYear()):
            dayOfMonthToday = today.getDate()

        startWeekDay = self.getDateTimeService().getStartWeekDay(
                self._focusedDate)
        daysInMonth = DateTimeService.getNumberOfDaysInMonth(
                self._focusedDate)
        ...
        while weekOfMonth < 7:
            weekOfMonth += 1
            weekNumberProcessed = [False, False, False, False, False,
                    False, False]
            ...
            while dayOfWeek < 7:
                ...
                if not (weekOfMonth == 1 and dayOfWeek < startWeekDay):
                    ...
                    self._days.setWidget(weekOfMonth,
                            firstWeekdayColumn + dayOfWeek, day)

                    # ISO week numbers if requested
                    if not weekNumberProcessed[weekOfMonth]:
                        self._days.getCellFormatter().setVisible(weekOfMonth,
                                weekColumn, self.isShowISOWeekNumbers())
                        if self.isShowISOWeekNumbers():
                            baseCssClass = (VDateField.CLASSNAME
                                    + '-calendarpanel-weeknumber')
                            weekCssClass = baseCssClass
                            weekNumber = DateTimeService.getISOWeekNumber(curr)
                            self._days.setHTML(weekOfMonth, 0,
                                    '<span class=\"' + weekCssClass + '\"'
                                    + '>' + weekNumber + '</span>')
    ...
    def renderCalendar(self):
        ...
        if (self.getResolution() <= VDateField.RESOLUTION_MONTH
                and self._focusChangeListener is not None):
            self._focusChangeListener.focusChanged(Date(
                    self._focusedDate.getTime()))
        ...
        if self.isTimeSelectorNeeded() and self._time is None:
            ...
            self.getFlexCellFormatter().setStyleName(2, 0,
    ...
    def handleKeyPress(self, event):
        ...
        """
        if (self._time is not None
                and self._time.getElement().isOrHasChild(
        ...
        if (keycode == KeyboardListener.KEY_TAB
                and event.getNativeEvent().getShiftKey()):
            if self.onTabOut(event):
                return

        # Handle the navigation
        if (self.handleNavigation(keycode,
                event.getNativeEvent().getCtrlKey()
                        or event.getNativeEvent().getMetaKey(),
    ...
    def handleNavigationMonthMode(self, keycode, ctrl, shift):
        ...
        if ctrl:
            ...
            return True
                ...
                or (keycode == KeyboardListener.KEY_TAB)):
    ...
    def getResetKey(self):
        ...
        selection. By default this is backspace but it can be overriden to
        change the key to whatever you want.
        """
        return KeyboardListener.KEY_BACKSPACE


    def getSelectKey(self):
        """Returns the select key which selects the value. By default this is
        the enter key but it can be changed to whatever you like by overriding
        this method.
        """
    ...
    def getCloseKey(self):
        """Returns the key that closes the popup window if this is a
        VPopopCalendar. Else this does nothing. By default this is the Escape
        key but you can change the key to whatever you want by overriding this
        method.
        """
    ...
    def getForwardKey(self):
        """The key that selects the next day in the calendar. By default this
        is the right arrow key but by overriding this method it can be changed
    ...
    def getBackwardKey(self):
        """The key that selects the previous day in the calendar. By default
        this is the left arrow key but by overriding this method it can be
    ...
    def getNextKey(self):
        """The key that selects the next week in the calendar. By default this
        is the down arrow key but by overriding this method it can be changed
        to whatever you like.
        """
    ...
    def getPreviousKey(self):
        """The key that selects the previous week in the calendar. By default
        this is the up arrow key but by overriding this method it can be
        changed to whatever you like.
        """
    ...
    def onMouseDown(self, event):
        ...
        if isinstance(event.getSource(), VEventButton):
    ...
    def setDate(self, currentDate):
        ...
        if ((oldFocusedValue is None) or (self._value is None)
                or (oldFocusedValue.getYear() != self._value.getYear())
                or (oldFocusedValue.getMonth() != self._value.getMonth())):
            self.renderCalendar()
        else:
    ...
    def setFocusOutListener(self, listener):
        """A focus out listener is triggered when the panel loosed focus. This
        can happen either after a user clicks outside the panel or tabs out.
    ...
    def setSubmitListener(self, submitListener):
        """The submit listener is called when the user selects a value from
        the calender either by clicking the day or selects it by keyboard.
    ...
    def setFocusChangeListener(self, listener):
        """The given FocusChangeListener is notified when the focused date
        changes by user either clicking on a new date or by using the
        keyboard.
    ...
    def setTimeChangeListener(self, listener):
        """The time change listener is triggered when the user changes the
        time.
        """
...
class VTime(FlowPanel):#, ChangeHandler):
    """TimeSelector is a widget consisting of list boxes that modified the
    Date object that is given for.
    """
    ...
    def createListBox(self):
        ...
        lb.addBlurHandler(self._cp)
        lb.addFocusHandler(self._cp)
        return lb
    ...
    def buildTime(self):
        ...
        if self._cp.getResolution() >= VDateField.RESOLUTION_MIN:
        ...
        if self._cp.getResolution() >= VDateField.RESOLUTION_SEC:
        ...
        if self._cp.getResolution() == VDateField.RESOLUTION_MSEC:
        ...
        if self._cp.isReadonly():
            h = 0
            if self._cp._value is not None:
                h = self._cp._value.getHours()
        ...
        if self._cp.getResolution() == VDateField.RESOLUTION_HOUR:
            self.add(VLabel(delimiter + '00'))  # o'clock
        ...
        if self.getDateTimeService().isTwelveHourClock():
            ...
            if self._cp.isReadonly():
                i = 0
                if self._cp._value is not None:
                    i = 0 if self._cp._value.getHours() < 12 else 1
    ...
    def updateTimes(self):
        ...
        selected = True
        if self._cp._value is None:
            self._cp._value = Date()
            selected = False
        ...
        if self.getDateTimeService().isTwelveHourClock():
            ...
            self._hours.setSelectedIndex(self._cp._value.getHours())

        if self._cp.getResolution() >= VDateField.RESOLUTION_MIN:
            self._mins.setSelectedIndex(self._cp._value.getMinutes())

        if self._cp.getResolution() >= VDateField.RESOLUTION_SEC:
            self._sec.setSelectedIndex(self._cp._value.getSeconds())
        ...
        if self.getDateTimeService().isTwelveHourClock():
            self._ampm.setSelectedIndex(0 if self._cp._value.getHours() < 12 else 1)

        self._hours.setEnabled(self._cp.isEnabled())

        if self._mins is not None:
            self._mins.setEnabled(self._cp.isEnabled())

        if self._sec is not None:
            self._sec.setEnabled(self._cp.isEnabled())

        if self._msec is not None:
            self._msec.setEnabled(self._cp.isEnabled())

        if self._ampm is not None:
            self._ampm.setEnabled(self._cp.isEnabled())


    def getMilliseconds(self):
        return DateTimeService.getMilliseconds(self._cp._value)


    def getDateTimeService(self):
        if self._cp._dateTimeService is None:
            self._cp._dateTimeService = DateTimeService()
        return self._cp._dateTimeService
    ...
    def onChange(self, event):
        ...
        if event.getSource() == self._hours:
            ...
            self._cp._value.setHours(h)
            if self._cp._timeChangeListener is not None:
                self._cp._timeChangeListener.changed(h,
                        self._cp._value.getMinutes(),
                        self._cp._value.getSeconds(),
                        DateTimeService.getMilliseconds(self._cp._value))
            event.preventDefault()
            event.stopPropagation()
        elif event.getSource() == self._mins:
            m = self._mins.getSelectedIndex()
            self._cp._value.setMinutes(m)
            if self._cp._timeChangeListener is not None:
                self._cp._timeChangeListener.changed(self._cp._value.getHours(),
                        m, self._cp._value.getSeconds(),
                        DateTimeService.getMilliseconds(self._cp._value))
            event.preventDefault()
            event.stopPropagation()
        elif event.getSource() == self._sec:
            s = self._sec.getSelectedIndex()
            self._cp._value.setSeconds(s)
            if self._cp._timeChangeListener is not None:
                self._cp._timeChangeListener.changed(self._cp._value.getHours(),
                        self._cp._value.getMinutes(), s,
                        DateTimeService.getMilliseconds(self._cp._value))
            event.preventDefault()
            event.stopPropagation()
        elif event.getSource() == self._msec:
            ms = self._msec.getSelectedIndex()
            DateTimeService.setMilliseconds(self._cp._value, ms)
            if self._cp._timeChangeListener is not None:
                self._cp._timeChangeListener.changed(self._cp._value.getHours(),
                        self._cp._value.getMinutes(),
                        self._cp._value.getSeconds(), ms)
            event.preventDefault()
            event.stopPropagation()
        elif event.getSource() == self._ampm:
            h = (self._hours.getSelectedIndex()
                    + (self._ampm.getSelectedIndex() * 12))
            self._cp._value.setHours(h)
            if self._cp._timeChangeListener is not None:
                self._cp._timeChangeListener.changed(h,
                        self._cp._value.getMinutes(),
