...
class VTextualDate(VDateField, Paintable, Field, ChangeHandler, ContainerResizedListener, Focusable, SubPartAware):
    _PARSE_ERROR_CLASSNAME = CLASSNAME + '-parseerror'
    _text = None
    _formatStr = None
    _width = None
    _needLayout = None
    ...
    def __init__(self):
        ...
        _VTextualDate_this = self

        class _0_(FocusHandler):

            def onFocus(self, event):
                self.text.addStyleName(VTextField.CLASSNAME + '-' + VTextField.CLASSNAME_FOCUS)
                if self.prompting:
                    self.text.setText('')
                    self.setPrompting(False)
                if (
                    self.getClient() is not None and self.getClient().hasEventListeners(_VTextualDate_this, EventId.FOCUS)
                ):
        ...
        class _1_(BlurHandler):
            ...
            def onBlur(self, event):
                self.text.removeStyleName(VTextField.CLASSNAME + '-' + VTextField.CLASSNAME_FOCUS)
                value = self.getText()
                self.setPrompting(self.inputPrompt is not None and (value is None) or ('' == value))
                if self.prompting:
                    self.text.setText('' if self.readonly else self.inputPrompt)
                if (
                    self.getClient() is not None and self.getClient().hasEventListeners(_VTextualDate_this, EventId.BLUR)
                ):
    ...
    def setWidth(self, newWidth):
        if (
            ...
            if BrowserInfo.get().isIE6():
                ...
                self.DOM.setElementProperty(self._text.getElement(), 'size', '1')
            ...
            if BrowserInfo.get().isIE6():
                # revert IE6 hack
                self.DOM.setElementProperty(self._text.getElement(), 'size', '')
