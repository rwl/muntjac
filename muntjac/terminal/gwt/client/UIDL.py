...
class UIDL(JavaScriptObject):
    ...
    def getChildIterator(self):
        ...
        class _0_(Iterator):
            ...
            def next(self):
                if self.hasNext():
                    typeOfChild = UIDL_this.typeOfChild(PREINC(globals(), locals(), 'self._index'))
                    _0 = typeOfChild
                    _1 = False
                    while True:
                        if _0 == UIDL_this._CHILD_TYPE_UIDL:
                            _1 = True
                            childUIDL = UIDL_this.getChildUIDL(self._index)
                            return childUIDL
                        if (_1 is True) or (_0 == UIDL_this._CHILD_TYPE_STRING):
                            _1 = True
                            return UIDL_this.getChildString(self._index)
                        if (_1 is True) or (_0 == UIDL_this._CHILD_TYPE_XML):
                            _1 = True
                            return UIDL_this.getChildXML(self._index)
                        if True:
                            _1 = True
                            raise self.IllegalStateException('Illegal child  in tag ' + UIDL_this.getTag() + ' at index ' + self._index)
                        break
                return None

            def hasNext(self):
                count = UIDL_this.getChildCount()
