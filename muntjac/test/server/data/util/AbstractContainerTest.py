...
class AbstractContainerTest(TestCase):
    ...
    class AbstractEventCounter(object):
        ...
        def assertNone(self):
            ...
            Assert.assertEquals(self._lastAssertedEventCount, self._eventCount)

        def assertOnce(self):
            """Check that exactly one event has occurred since the previous assert
            call.
            """
            Assert.assertEquals(PREINC(globals(), locals(), 'self._lastAssertedEventCount'), self._eventCount)
    ...
    class ItemSetChangeCounter(AbstractEventCounter, ItemSetChangeListener):
    ...
    def validateContainer(self, container, expectedFirstItemId, expectedLastItemId, itemIdInSet, itemIdNotInSet, checkGetItemNull, expectedSize):
        indexed = None
        if isinstance(container, Container.Indexed):
            indexed = container
        ...
        # getItem
        if checkGetItemNull:
            self.assertNull(container.getItem(itemIdNotInSet))
        self.assertNotNull(container.getItem(itemIdInSet))
        # getContainerProperty
        for propId in container.getContainerPropertyIds():
            if checkGetItemNull:
                self.assertNull(container.getContainerProperty(itemIdNotInSet, propId))
            self.assertNotNull(container.getContainerProperty(itemIdInSet, propId))
    ...
    def testContainerOrdered(self, container):
        ...
        self.assertNotNull(newSecondItemId)
    ...
    def testContainerIndexed(self, container, itemId, itemPosition, testAddEmptyItemAt, newItemId, testAddItemAtWithId):
        self.initializeContainer(container)
        # indexOfId
        ...
        if testAddEmptyItemAt:
            ...
            Assert.assertEquals(len(container) - 1, container.indexOfId(newLastId))
            Assert.assertEquals(itemPosition + 2, container.indexOfId(itemId))
            Assert.assertEquals(newLastId, container.lastItemId())
            Assert.assertEquals(newLastId, container.getIdByIndex(len(container) - 1))
            Assert.assertEquals(itemId, container.getIdByIndex(itemPosition + 2))
        ...
        if testAddItemAtWithId:
            ...
            Assert.assertEquals(len(container) - 1, container.indexOfId(newItemId))
            Assert.assertEquals(itemPosition, container.indexOfId(itemId))
            Assert.assertEquals(newItemId, container.lastItemId())
            Assert.assertEquals(newItemId, container.getIdByIndex(len(container) - 1))
            Assert.assertEquals(itemId, container.getIdByIndex(itemPosition))
            Assert.assertTrue(container.removeItem(newItemId))
            Assert.assertFalse(container.containsId(newItemId))
    ...
    def testContainerFiltering(self, container):
        ...
        container.addContainerFilter(SimpleStringFilter(self.FULLY_QUALIFIED_NAME, 'ab', False, False))
        ...
        container.addContainerFilter(SimpleStringFilter(self.REVERSE_FULLY_QUALIFIED_NAME, 'ad', False, False))
    ...
    def testContainerSortingAndFiltering(self, sortable):
        ...
        # Filter by "contains ab"
        filterable.addContainerFilter(SimpleStringFilter(self.FULLY_QUALIFIED_NAME, 'ab', False, False))
