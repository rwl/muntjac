...
class ExampleUtil(object):
    ...
    _locales = ['fi', 'FI', 'Finnish', 'de', 'DE', 'German', 'en', 'US', 'US - English', 'sv', 'SE', 'Swedish']
    _hardware = ['Desktops', 'Dell OptiPlex GX240', 'Dell OptiPlex GX260', 'Dell OptiPlex GX280', 'Monitors', 'Benq T190HD', 'Benq T220HD', 'Benq T240HD', 'Laptops', 'IBM ThinkPad T40', 'IBM ThinkPad T43', 'IBM ThinkPad T60']
    ...
    def getPersonContainer(cls):
        ...
        while True:
            ...
            if not (i < 50):
                break
            fn = cls._firstnames[r.nextDouble() * len(cls._firstnames)]
            ln = cls._lastnames[r.nextDouble() * len(cls._lastnames)]
            id = fn + ln
            item = contactContainer.addItem(id)
            if item is not None:
                i += 1
    ...
    def getNameContainer(cls):
        ...
        while True:
            ...
            if item is not None:
                i += 1
                item.getItemProperty(cls.PERSON_PROPERTY_NAME).setValue(fn + ' ' + ln)
        return contactContainer
    ...
    def getLocaleContainer(cls):
        ...
        while True:
            if _0 is True:
                ...
                i += 1
            if not (i < len(cls._locales)):
                break
            id = cls._locales[i][2]
            item = localeContainer.addItem(id)
            item.getItemProperty(cls.locale_PROPERTY_LOCALE).setValue(Locale(cls._locales[i][0], cls._locales[i][1]))
            item.getItemProperty(cls.locale_PROPERTY_NAME).setValue(cls._locales[i][2])
    ...
    def fillIso3166Container(cls, container):
        ...
        container.addContainerProperty(cls.iso3166_PROPERTY_FLAG, Resource, None)
        _0 = True
        i = 0
        while True:
            if _0 is True:
                ...
                i += 1
            if not (i < cls.iso3166.length):
                break
            name = cls.iso3166[POSTINC(globals(), locals(), 'i')]
            id = cls.iso3166[i]
            item = container.addItem(id)
            item.getItemProperty(cls.iso3166_PROPERTY_NAME).setValue(name)
            item.getItemProperty(cls.iso3166_PROPERTY_SHORT).setValue(id)
            item.getItemProperty(cls.iso3166_PROPERTY_FLAG).setValue(ThemeResource('../sampler/flags/' + id.toLowerCase() + '.gif'))
    ...
    def getHardwareContainer(cls):
        item = None
        itemId = 0
        # Increasing numbering for itemId:s
        ...
        while True:
            if _0 is True:
                ...
                i += 1
            if not (i < len(cls._hardware)):
                break
            # Add new item
            item = hwContainer.addItem(itemId)
            # Add name property for item
            item.getItemProperty(cls.hw_PROPERTY_NAME).setValue(cls._hardware[i][0])
            # Allow children
            ...
            while True:
                if _1 is True:
                    ...
                    j += 1
                if not (j < cls._hardware[i].length):
                    break
                if j == 1:
                    item.getItemProperty(cls.hw_PROPERTY_ICON).setValue(ThemeResource('../runo/icons/16/folder.png'))
                # Add child items
                item = hwContainer.addItem(itemId)
                item.getItemProperty(cls.hw_PROPERTY_NAME).setValue(cls._hardware[i][j])
    ...
    def addOrderToContainer(cls, container, description, quantity, price):
        ...
        item.getItemProperty(cls.ORDER_DESCRIPTION_PROPERTY_ID).setValue(description)
        item.getItemProperty(cls.ORDER_QUANTITY_PROPERTY_ID).setValue(quantity)
        item.getItemProperty(cls.ORDER_UNITPRICE_PROPERTY_ID).setValue(NumberFormat.getCurrencyInstance().format(price))
        item.getItemProperty(cls.ORDER_ITEMPRICE_PROPERTY_ID).setValue(NumberFormat.getCurrencyInstance().format(price * quantity))
