...
class AbstractComponent(Component, MethodEventSource):
    ...
    def __init__(self):
        ...
        self._sizePattern = re.compile('^(-?\\d+(\\.\\d+)?)(%|px|em|ex|in|cm|mm|pt|pc)?$')
    ...
    def setStyleName(self, style):
        # Sets the component's style. Don't add a doc string here, we use the
        # default documentation from implemented interface.
    ...
    def getCaption(self):
        # Get's the component's caption. Don't add a doc string here, we use
    ...
    def setParent(self, parent):
        ...
        if parent is not None and self._parent is not None:
            raise ValueError, __class__.__name__ + ' already has a parent.'
    ...
    def fireRequestRepaintEvent(self, alreadyNotified):
        ...
        if not self._repaintRequestListenersNotified:
            ...
            if self._repaintRequestListeners is not None \
                    and not self._repaintRequestListeners.isEmpty():
                ...
                for i in range(len(listeners)):
                    ...
                    if listeners[i] not in alreadyNotified:
    ...
    def getListeners(self, eventType):
        ...
        @return A collection with all registered listeners. Empty if no listeners
                are found.
        """
        if RepaintRequestEvent in eventType.__class__.__mro__:  # TODO check isAssignableFrom() translation
    ...
    def fireComponentErrorEvent(self):
        ...
        """
        self.fireEvent(ComponentErrorEvent(self.getComponentError(), self))
...
class ComponentErrorHandler(object):
