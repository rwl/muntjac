...
#    LOAD_STYLE = LoadStyle.EAGER
    ...
    def addComponent(self, *args):
        ...
        if nargs == 1:
            ...
            while not done and index < len(self._areas):
                ...
                if (existingArea.row1 >= row1 and existingArea.column1 > column1) \
                        or (existingArea.row1 > row1):
                    self._areas.append(index, area)
                    self._components.append(index, component)
                    done = True
                index += 1
        ...
        for cury in range(self._rows):
            ...
            for curx in range(self._cols):
                ...
                if area is not None and area.row1 == cury \
                    ...
                    alignmentsArray[index] = str(self.getComponentAlignment( area.getComponent() ).getBitMask())
                    index += 1
    ...
    def getComponent(self, x, y):
        ...
        """
        for area in self._areas:
            if area.getColumn1() <= x \
                    and x <= area.getColumn2() \
                    and area.getRow1() <= y \
                    and y <= area.getRow2():
...
class OverlapsException(RuntimeError):
    ...
    def getMessage(self):
        ...
        sb.write(component)
        sb.write('( type = ')
        sb.write(component.getClass().getName())
